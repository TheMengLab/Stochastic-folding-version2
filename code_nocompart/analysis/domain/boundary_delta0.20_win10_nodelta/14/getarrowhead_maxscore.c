...
void main()
{
char inputfilename[N],outputfilename[N],scorefilename[N];
int i,j,k,statenum,linenum,*assign,bid,eid,lag,minlag;
double **matrix,**scorematrix,*scorelist,tempscore;
FILE *inputfile,*outputfile,*scorefile;
...
for(i=0;i<statenum;i++)
 ...
 if(assign[i] == 1)
  ...
  scorelist[i] = getscore(scorematrix,statenum,bid,eid);
  }
 }
...
for(i=0;i<statenum;i++)
 ...
 for(j=minlag;j<lag;j++)
  ...
  if(assign[i] == 1)
   ...
   tempscore = getscore(scorematrix,statenum,bid,eid);

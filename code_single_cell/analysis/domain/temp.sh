...
filenum=`ls ../conf/mergepoint/[1-9]*.dat | nl | tail -n 1 | awk '{print $1}'`

for id in `seq ${chrid} ${chrid}`
do

nl ../../prepare/assign/detail/chrassign_10kb.dat | awk '{print $1,$2}' | grep " "$[id-1]$ > temp_chr${id}.dat
...
do
head -n $[eid] ../conf/mergepoint/${i}.dat | tail -n $[eid-sid+1] > tempcoor/${i}_chr${id}.dat
done

...
void main()
{
char startpointfilename[N],assignfilename[N],outputfilename[N];
//char outputfilename[N];
int i,j,k,l,statenum,**assign,collisionbool,*trapcount,seed,chrnum;
double **coor,radius,len,*vector,distsq,*center,chromatinlen,**startpoint;
FILE *outputfile,*startpointfile,*assignfile;

printf("Input filename for starting point:\n");
scanf("%s",startpointfilename);
...
chrnum = getlinenum(startpointfilename);
...
startpoint = doublematrix(chrnum,DIM);
...
startpointfile = openfile(startpointfilename,'r');
for(i=0;i<chrnum;i++)
 {
 for(j=0;j<DIM;j++)
  fscanf(startpointfile,"%lf",&startpoint[i][j]);
 }
fclose(startpointfile);

for(i=0;i<DIM;i++)
 {
 coor[0][i] = startpoint[0][i];
 center[i] = 0;
 }
...
// printf("%d\n",i);
 ...
 if(assign[i][1] == assign[i-1][1])
  ...
  {
  for(j=0;j<DIM;j++)
   coor[i][j] = startpoint[assign[i][1]][j];
  }
...
//check collision
 ...
 if(collisionbool == 1) 
  {
//  printf("%lf %lf %lf %lf %lf %lf\n",coor[i-1][0],coor[i-1][1],coor[i-1][2],coor[i][0],coor[i][1],coor[i][2]);
  trapcount[i] ++;

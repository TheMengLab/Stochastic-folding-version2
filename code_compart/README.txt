...
1.Introduction
The repository contains the codes and script for simulating 3D chromosome structures from populated DNA accessibility data based on stochastic folding model. The code is written by C and the script is written in bash
Both the source code and the compiled code are presented in the repository.
The input data for the codes contains three parts: (1)the populated DNA accessibility data(we use ATAC-seq of K562 as a demonstration here, deposited as ./prepare/data_raw/ATAC_data.dat), (2) the sequence length of each chromosome in the reference genome (we use hg19 genome, deposited as ./prepare/data_raw/hg19.chrom.sizes) and (3) the compartment assignment of each 10-kb segments for different chromosome, deposited as ./prepare/assign/detail/assign1_10kb_compbk.dat (chr 1),  ./prepare/assign/detail/assign2_10kb_compbk.dat (chr 2), and etc. In the compartment assignment file ./prepare/assign/detail/assign1_10kb_compbk.dat, the i-th row represent the compartment type of the i-th 10-kb segment(0 for compartment B and 1 for compartment A)
...
(1)ATAC_data.dat: DNA accessibility data presented in four column, ie chrid, start_pos, end_pos, signal value. Here we use ATAC-seq data as an example. The data is presented in four column as below:
chr1    100503131       100504554       1.83651
chr1    100503131       100504554       5.32597
chr1    100503131       100504554       4.66917
since we only include autosomes and X chromosome in our simulation, the DNA accessibility signal for other chromosomes are not use in our calculation(it will be ok if you put these data in the file).

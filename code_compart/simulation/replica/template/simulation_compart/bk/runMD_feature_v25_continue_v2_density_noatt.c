...
double getforce_feature_all_noneighbor_pair_soft_sepattrep_darkatt_allpair_chr_v3_density(double **coor,int pointnum,double bondlen,double kb,double contactcutoff,double sigma,double epsi,int **targetlist,int featurenum,int *targetlen,double **forcelist,double sphereradius,int **pairlist,int *pairlen,int **allpairlist,int *allpairlen,int *chrassign,double *density)
 {
 int i,j,k,l,tempid,id1,id2,testcount,temp,avebondlen;
 double distsq,dist,*tempvector,coef,force,tempcutoff,Fmax,contactcutoffsq,tempFmax,tempF,WCAcutoff,WCAcutoffsq,factor,sigmasq,sigma3,sigma5,sigma11,dist6,sigma6,sigma12,exfactor,exfactor6;
...
//     }
    if((distsq<WCAcutoff))
     ...
     force = 0.0001*4*epsi*(12*sigma12/(dist6*dist6)-6*sigma6/dist6);
...
//    force = density[i]*density[tempid]*SCALE*4*epsi*(12*sigma12/(4096*dist6*dist6)-6*sigma6/(64*dist6));
    force = density[i]*density[tempid]*SCALE*4*epsi*(12*sigma12/(exfactor6*exfactor6*dist6*dist6)-6*sigma6/(exfactor6*dist6));
...
void main()
{
char inputfilename[N],assignfilename[N],outputfilename[N],densityfilename[N];
int i,j,k,l,m,n,pointnum,featurenum,*assign,**featureassign,stepnum,interval,seed,assignmax,**targetlist,*targetlen,**pairlist,*pairlen,pairinterval,shrinkmax,shrinkinterval,shrinkendbool,**randompairlist,*randompairlen,sepprob,**contacttime,**allpairlist,*allpairlen,*chrassign,tempmax,maxlen,*tempassign,nextstepnum;
double **coor,**velvector,**velvector2,**forcelist,radius,kT,bondlen,kb,beadradius,Fmax,dt,dtp,t,sphereradius,chrgyr,pointmass,sigma,epsi,distcutoff,factor,gyr_target,*density,densitybase,center[DIM];
FILE *inputfile,*assignfile,*outputfile,*densityfile;
...
sphereradius = beadradius*pow(pointnum,0.3333)*2.5;
